...
export async function getSettingsFileExists(): Promise<{ exists: boolean }> {
  const response = await apiFetch('/api/settings/exists')
...
export async function getProxySettings(): Promise<Record<string, unknown>> {
  const response = await apiFetch('/api/settings/proxy')
  return await response.json()
}
...
export const openFolderInExplorer = async (path: string) => {
  const response = await apiFetch('/api/open_folder_in_explorer', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const getFileServiceUrl = (filePath: string) => {
  return resolveApiUrl(`/api/serve_file?file_path=${encodeURIComponent(filePath)}`)
}
...
export const getMyAssetsDirPath = async () => {
  const response = await apiFetch('/api/settings/my_assets_dir_path')
